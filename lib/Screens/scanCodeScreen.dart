...
class ScanCodeScreen extends StatefulWidget {
  ...
  ScanCodeScreen(this.userModel, this.active, this.censerModel,this.iconModelVentas,this.iconmodellistTienda);
...
class _ScanCodeScreenState extends State<ScanCodeScreen> {
 ...
 List<ActivacionesTotal>ventasTotal=[];
  ...
  preScaneo()async{
     scaneoModel=await FetchData().getScaneos(widget.censerModel.id,widget.userModel.id);
...
Toast.show("Si no ha pasado 1 hora este usuario ya se escaneo previamente no se contabilizara", context, duration: Toast.LENGTH_LONG);
      }else{
  ...
  scaneoTrue()async{
    if(widget.active==true){
      ...
      DateTime now =DateTime.now();
       bool erroguardar=await QuerysService().SaveGeneralInfoScaneos(reference: "Scaneos", id:id_variable, collectionValues:ScaneosModel().toJsonBodyScaner(
        id_variable,
  ...
  Widget build(BuildContext context) {
    ...
    widget.censerModel.activacionesRestantes==0?_showAlertVentaMaxima():
                      _showAlertActive();
                    },
                    child: Container(
                      width: 200.0,
                  ...
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 20.0),
                    child: Text(
                      widget.censerModel.activacionesRestantes==0? "Alcanzaste el numero maximo de activaciones, Procede a pagar" : "Te quedan ${widget.censerModel.activacionesRestantes} actvaciones mas por realizar",
                      style: TextStyle(
                          fontSize: 16.0,
    ...
    AlertDialog alertDialog = AlertDialog(
      ...
      actions: <Widget>[
        ...
        FlatButton(
          ...
          context,
          MaterialPageRoute(
              builder: (context) => ViewPago (widget.censerModel,widget.iconModelVentas,ventasTotal[0])
          )
      );
          },
        ),
  ...
  _activeUser(){
    ...
    QuerysService().updateUserDataServices(idUser: widget.userModel.id, context: context, collectionValues: nextTime, errorFunction: _errorFunction, succesfulFunction: _succesfulFunction);
  ...
  _succesfulFunction()async{
    ...
    QuerysService().SaveSale(idSale: idSale_, errorFunction: _errorFunction, function: _sucessTotal, context: context, collectionValues: {
      'id' : idSale_,
    ...
    bool erroguardar=await QuerysService().UpdateCenserVentas(reference: FirebaseReferencias.REFERENCE_ActivacionesTotalCamionero, id:widget.censerModel.id, collectionValues:ActivacionesTotal().toJsonBodyActualizar(
        ...
        bool erroguardarCamion=await QuerysService().UpdateCenserCamion(reference: FirebaseReferencias.REFERENCE_CENSERS, id:widget.censerModel.id, collectionValues:CenserModel().toJsonBodyCamionActivacion(
    ...
    AlertDialog alertDialog = AlertDialog(
      ...
      actions: <Widget>[
        FlatButton(
          ...
          onPressed: ()async {
           CenserModel adminmodel = await FetchData().getCamionero(widget.censerModel.id);
