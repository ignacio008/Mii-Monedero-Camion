...
class ScreenPago extends StatefulWidget {
  ...
  ScreenPago(this.pagos,this.censerModel,this.iconModelVentas);
 ...
 double numPago=200.00;
  _createOrderPago()async{
      DateTime now = DateTime.now();
        ...
        // bool pagoError=await QuerysService().SavePago(reference: "PagoCamionero", id:id_variable, collectionValues:PagoModel().toJsonBody(
        // id_variable,
        // idCamionero,
        // widget.pagos,
        // now,
             ...
             Padding(
              ...
              child: Center(
                ...
                child:Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                     _loading? Text("Su pago es de: ", style:TextStyle(fontSize: 20)):
                     Text("Tu pago fue de: ", style:TextStyle(fontSize: 20)),
                    const SizedBox(height:6.0),
                    Text("\$ ${widget.pagos} MXN", style:TextStyle(fontSize: 20)),
                    ...
                    _loading?
                     ...
                     RaisedButton(
                       ...
                       color:Colors.green,
                       child:Text("Gracias por tu recarga", style:TextStyle(
                         color:Colors.white,
                       )),
                       onPressed: ()async{
  ...
  void onData(DocumentSnapshot event) async{
    ...
    // }
     ...
     if(event.data()['status']=='Aprobado'||event.data()['status']=='Pendiente'){
       setState(() {
         _loading=false;
       ...
       if(event.data()['preference_id']!=null){
         var  response= await MercadoPagoMobileCheckout.startCheckout("TEST-32a3ac49-f045-41ac-95df-c9fa000f6eea", event.data()['preference_id']);
       print(response);
       ...
       if( response.status =='approved'){
        ...
        if(erroguardarCamionero){
          ...
          var orderRef =FirebaseFirestore.instance.collection("EstadoPagoCamionero").doc(id_variable);
      ...
      event.data()['preference_id'],  ));
         ...
         event.reference.update({
          //  'payment':response,
           'status':'Pendiente',
           'message':'Tu pedido se enviara en 24 hrs despues de acreditar tu pago',
         });
